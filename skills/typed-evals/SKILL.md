---
name: typed-evals
description: Use the typed-evals CLI to run, inspect, and diff LLM evals when iterating on agent code, prompts, models, or retrieval. Use this skill whenever the user asks you to improve, optimize, debug, fix, or iterate on an LLM task — especially if `*.eval.ts` files or a `.typed-evals/runs.db` exist in the project, even if they don't mention typed-evals or evals by name. Triggers include "make my agent better", "improve the prompt", "the score dropped", "why did this regress", "what's failing", "tune this scorer", "iterate on this eval", or any request to change LLM-facing code in a project that already has evals set up.
---

# Working with typed-evals

typed-evals records every LLM eval run to SQLite and exposes a JSON CLI. Use it to close the loop on iterations: change → run → diff → keep or revert.

## Mental model

- **Every change → run.** `typed-evals run --json` is the equivalent of `npm test --json` — always parse stdout to learn the score.
...
Use this sequence whenever you're improving an LLM task in a project with typed-evals.

1. **Orient.** See what's already been tried:
   ```bash
   typed-evals list --versions --limit 10
   ```
...
3. **Establish a baseline if there isn't one.** If `list --versions` is empty, run once and tag it before making any changes:
   ```bash
   typed-evals run --note "baseline" --json
   ```
...
| Command | When to use |
|---|---|
| `typed-evals run --json` | After every change. Prints summary; exit 1 if any test failed. |
| `typed-evals run --note "X" --json` | Mark this run as a version. Use sparingly — only intentional checkpoints. |
| `typed-evals show latest --full` | Inspect per-row data of the most recent run. |
| `typed-evals show <runId>` | Pull a specific run as JSON (summary by default). |
| `typed-evals diff prev-version` | Compare `latest` against the most recent version. Exit 1 on regression. |
| `typed-evals diff <a> <b>` | Compare any two runs. |
| `typed-evals list --versions` | History of intentional iterations. |
...
typed-evals run --json | jq '.score'

# per-scorer means for one run
typed-evals show latest | jq '.evals[] | {name, scorers}'
...
typed-evals run --json | jq -r '.runId'
```
...
- **Don't run `typed-evals dev`.** That's the interactive watcher for humans. From an agent context, use `run --json` directly.
- **Don't reach into `.typed-evals/runs.db` with raw SQL** unless something is broken. The CLI covers the common reads and gives you stable JSON shapes.
